...
# --- workspace-registered OAuth apps --------------------------------------------------------------


@pytest.mark.parametrize("vendor", ["gmail"], indirect=True)
async def test_workspace_can_register_its_own_oauth_app_and_then_connect(
    client: Any, vendor: VendorMock, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Gmail has no official MCP server: without a Google app it is unavailable, and the page
    tells the workspace owner exactly how to register one. Once saved, Connect works."""
    from app.core.config import get_settings

    monkeypatch.setattr(get_settings(), "oauth_google_client_id", "")
    monkeypatch.setattr(get_settings(), "oauth_google_client_secret", "")
    await signup(client)
    detail = (await client.get("/api/v1/integrations/providers/gmail")).json()["data"]
    assert detail["connect_methods"] == [] and detail["workspace_app"] is None
    assert detail["settings_prefix"] == "google"
    assert detail["redirect_uri"] == "http://localhost:3000/api/v1/oauth/gmail/callback"
    assert detail["oauth_setup"]["console_url"].startswith("https://console.cloud.google.com")
    assert any("redirect URI" in step for step in detail["oauth_setup"]["steps"])

    missing = await client.put(
        "/api/v1/integrations/providers/gmail/oauth-app",
        json={"client_id": "test-id"},
        headers=ORIGIN,
    )
    assert missing.status_code == 422
    assert missing.json()["error"]["details"]["fields"] == {"client_secret": ["Required"]}
    ...
    assert saved.json()["data"]["client_id"] == "test-id"
    assert "test-secret" not in saved.text

    detail = (await client.get("/api/v1/integrations/providers/gmail")).json()["data"]
    assert detail["connect_methods"] == ["oauth"] and detail["configured"] is True
    assert detail["workspace_app"]["configured_by_me"] is True
    # The same Google app now serves Calendar and Drive too.
    listing = {
        p["id"]: p for p in (await client.get("/api/v1/integrations/providers")).json()["data"]
    }
    assert listing["google_calendar"]["connect_methods"] == ["oauth"]
    assert listing["google_drive"]["connect_methods"] == ["oauth"]

    # The full flow runs with the workspace's credentials (the mock asserts client_secret).
    detail = await connect_via_oauth(client, "gmail", CASES["gmail"])
    assert detail["connection"]["status"] == "connected"
    audit = (await client.get("/api/v1/audit")).json()["data"]
    assert ("gmail", "configure_oauth_app") in {(a["provider"], a["action"]) for a in audit}

    # Secrets stay encrypted and removing the app hides Connect again.
    from sqlalchemy import select

    from app.db.session import get_session_factory
    from app.models.integration import TenantOAuthApp

    async with get_session_factory()() as db:
        row = await db.scalar(select(TenantOAuthApp))
        assert row is not None and "test-secret" not in row.client_secret_encrypted
    assert (
        await client.delete("/api/v1/integrations/providers/gmail/oauth-app", headers=ORIGIN)
    ).status_code == 200
    detail = (await client.get("/api/v1/integrations/providers/gmail")).json()["data"]
    assert detail["workspace_app"] is None and detail["connect_methods"] == []


async def test_workspace_apps_are_isolated_per_tenant(client: Any) -> None:
    await signup(client, "ada@example.com")
    await client.put(
        "/api/v1/integrations/providers/slack/oauth-app",
        json={"client_id": "ada-app", "client_secret": "s"},
        headers=ORIGIN,
    )
    client.cookies.clear()
    await signup(client, "bob@example.com")
    detail = (await client.get("/api/v1/integrations/providers/slack")).json()["data"]
    assert detail["workspace_app"] is None and detail["connect_methods"] == []
