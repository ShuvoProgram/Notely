...
async def connect(
    client: Any, provider_id: str, case: dict[str, Any], scopes: list[str] | None = None
) -> dict[str, Any]:
    return await connect_via_oauth(client, provider_id, case, scopes=scopes)
...
async def connect_via_oauth(
    ...
    params = {"scopes": ",".join(scopes)} if scopes else {}
    start = await client.get(f"/api/v1/oauth/{provider_id}/start-url", params=params)
